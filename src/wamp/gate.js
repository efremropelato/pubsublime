...
handlers[WAMP.HELLO] =  (ctx, session, message) => {
...
handlers[WAMP.AUTHENTICATE] =  (ctx, session, message) => {
...
handlers[WAMP.GOODBYE] =  (ctx, session, message) => {
  this.sendGoodbye(ctx)
}

handlers[WAMP.REGISTER] =  (ctx, session, message) => {
...
cmdAck[WAMP.REGISTER] =  (cmd) => {
  this.wampSend([WAMP.REGISTERED, cmd.id, cmd.qid])
}

handlers[WAMP.CALL] =  (ctx, session, message) => {
...
handlers[WAMP.CANCEL] =  (ctx, session, message) => {
  var id = message.shift()
  var opt = message.shift() || {}
  session.realm.doCancel(ctx, { wtype: WAMP.CANCEL, id, opt })
}

handlers[WAMP.UNREGISTER] =  (ctx, session, message) => {
  var id = message.shift()
...
handlers[WAMP.SUBSCRIBE] =  (ctx, session, message) => {
...
cmdAck[WAMP.SUBSCRIBE] =  (cmd) => {
  this.wampSend([WAMP.SUBSCRIBED, cmd.id, cmd.qid])
}

handlers[WAMP.UNSUBSCRIBE] =  (ctx, session, message) => {
  const id = message.shift()
...
cmdAck[WAMP.UNSUBSCRIBE] =  (cmd) => {
  if (cmd.id) { // do not send on disconnect
    this.wampSend([WAMP.UNSUBSCRIBED, cmd.id])
  }
}

handlers[WAMP.PUBLISH] =  (ctx, session, message) => {
  const id = message.shift()
  const opt = message.shift() || {}
  const uri = wampParse(message.shift())
...
cmdAck[WAMP.PUBLISH] =  (cmd) => {
  this.wampSend([WAMP.PUBLISHED, cmd.id, cmd.qid])
}

handlers[WAMP.ERROR] =  (ctx, session, message) => {
